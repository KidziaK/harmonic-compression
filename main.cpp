...
int main()
{
    ...
    constexpr int n = 16;
    constexpr size_t total_batches_to_process = 50;
    ...
    PC point_cloud_double = PC::from_e57("../data/StSulpice-Cloud-50mm.e57");
    ...
    point_cloud.x_coords.reserve(point_cloud_double.x_coords.size());
    point_cloud.y_coords.reserve(point_cloud_double.y_coords.size());
    point_cloud.z_coords.reserve(point_cloud_double.z_coords.size());
    for(const auto& val : point_cloud_double.x_coords) point_cloud.x_coords.push_back(static_cast<T>(val));
    for(const auto& val : point_cloud_double.y_coords) point_cloud.y_coords.push_back(static_cast<T>(val));
    for(const auto& val : point_cloud_double.z_coords) point_cloud.z_coords.push_back(static_cast<T>(val));
    ...
    reconstructed_full_double.x_coords.reserve(reconstructed_full.x_coords.size());
    reconstructed_full_double.y_coords.reserve(reconstructed_full.y_coords.size());
    reconstructed_full_double.z_coords.reserve(reconstructed_full.z_coords.size());
    for(const auto& val : reconstructed_full.x_coords) reconstructed_full_double.x_coords.push_back(static_cast<double>(val));
    for(const auto& val : reconstructed_full.y_coords) reconstructed_full_double.y_coords.push_back(static_cast<double>(val));
    for(const auto& val : reconstructed_full.z_coords) reconstructed_full_double.z_coords.push_back(static_cast<double>(val));

    reconstructed_full_double.to_e57("/home/mikolaj/Documents/github/harmonic-compression/data/reconstructed_full.e57");
    std::cout << "Full reconstruction saved to reconstructed_full.e57" << std::endl;
